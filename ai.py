...
def fetch_price_df(ticker: str) -> pd.DataFrame:
    ...
    df = stock.get_historical_data()  # default: 7d @ 1m in your Stock class
    if df.empty:
        return df
...
def build_price_model(seq_len: int, num_features: int, num_targets: int):
    ...
    out = layers.Dense(num_targets, activation='linear')(x)  # Linear for regression (prices/volume)
    model = Model(inp, out, name="price_predictor_lstm")
...
def run_price_training(config):
    ...
    model = build_price_model(seq_len, num_features=X.shape[2], num_targets=len(target_cols))
...
def evaluate_price_model(config):
    ...
    if not os.path.exists(config['PRICE_MODEL_PATH']):
        ...
        return
    model = tf.keras.models.load_model(config['PRICE_MODEL_PATH'])
...
def predict_next_price(config):
    ...
    if next_ts is None:
        print("Next-step time: (could not infer; insufficient or non-datetime index)")
    else:
